...
/* Angel One instrument tokens (NSE segment) */
const ANGEL_TOKENS = {
  /* Indices */
  'NIFTY 50':'26000','BANKNIFTY':'26009','FINNIFTY':'26037',
  /* Equities */
  'RELIANCE':'2885','TCS':'11536','INFY':'1594','HDFCBANK':'1333',
  'ICICIBANK':'4963','WIPRO':'3787','SBIN':'3045','LT':'11483',
  'BAJFINANCE':'317','HCLTECH':'7229','MARUTI':'10999','ASIANPAINT':'236',
  'KOTAKBANK':'1922','AXISBANK':'5900','TITAN':'3506','ONGC':'2475',
  'NTPC':'11630','POWERGRID':'14977','ADANIENT':'25','ADANIPORTS':'15083'
};
/* Reverse: token -> symbol */
const TOKEN_TO_SYM = Object.fromEntries(Object.entries(ANGEL_TOKENS).map(([s,t])=>[t,s]));

/* Fallback simulated base prices */
const BASE_PRICES = {
  '26000':23201,'26009':49800,'26037':22100,
...
let isLive       = false;
let liveQuotes   = {};
let stockHistory = {};   /* sym -> [{t, p}] for sparklines/charts */
let sessionOpen  = {};   /* sym -> first seen price */
let pnlChart     = null;
let stockChart   = null;
let selectedStock= 'NIFTY 50';
...
function fmtINR(n, dp=2) {
  return (n<0?'-₹':'₹') + Math.abs(n).toLocaleString('en-IN',{minimumFractionDigits:dp,maximumFractionDigits:dp});
}
function fmtUSD(n, dp=2) {
  return (n<0?'-$':'$') + Math.abs(n).toLocaleString('en-US',{minimumFractionDigits:dp,maximumFractionDigits:dp});
}
function fmt(n, m, dp) {
  return m==='nse' ? fmtINR(n, dp??2) : fmtUSD(n, dp??( n<10?4:2 ));
}
...
function getLTP(sym) {
  /* Try: by symbol name, by angel token, then base */
  return liveQuotes[sym]
      ?? liveQuotes[ANGEL_TOKENS[sym]]
      ?? BASE_PRICES[sym]
      ?? BASE_PRICES[ANGEL_TOKENS[sym]]
      ?? 0;
}

function tickSimulated() {
  const keys = [...Object.keys(BASE_PRICES), ...Object.keys(liveQuotes)];
  [...new Set(keys)].forEach(k => {
    const base = BASE_PRICES[k] ?? liveQuotes[k] ?? 1;
    const vol  = base < 5 ? 0.008 : base < 100 ? 0.003 : 0.0015;
    liveQuotes[k] = Math.max(0.001, (liveQuotes[k]??base) + (Math.random()-0.495)*base*vol);
  });
}

function recordPriceHistory() {
  const syms = mode==='nse'
    ? ['NIFTY 50','BANKNIFTY','FINNIFTY',...NSE_SYMS]
    : CRYPTO_SYMS;
  const t = nowTime();
  syms.forEach(sym => {
    const p = getLTP(sym);
    ...
    stockHistory[sym].push({ t, p });
    if (stockHistory[sym].length > 120) stockHistory[sym].shift();
  });
}
...
async function loginAngelOne(cfg) {
  try {
    ...
    return { ok:false, msg: data.message || 'Login failed — check credentials' };
  } catch(e) {
...
async function fetchAngelQuotes() {
  ...
  const nseTokens = [...new Set([
    '26000','26009','26037',
    ...Object.values(ANGEL_TOKENS)
  ])];
  const nfoTokens = (S.nse.trades||[])
    .filter(t=>t.type==='options' && t.nfoToken).map(t=>t.nfoToken);
  const exchangeTokens = { NSE: nseTokens };
  ...
  try {
    ...
    if (data.status && data.data?.fetched) {
      data.data.fetched.forEach(q => {
        ...
        if (sym) liveQuotes[sym] = ltp;
      });
    } else {
      console.warn('Angel One quote API:', data.message);
    }
  } catch(e) {
    console.warn('fetchAngelQuotes failed:', e.message);
  }
}
...
async function onTick() {
  ...
  if (S[mode].pnlHistory.length > 120) S[mode].pnlHistory.shift();
  save();
  refreshDynamic();
}
...
function refreshDynamic() {
  ...
  if (activeTab==='chart')       updatePnlChart();
  if (activeTab==='stockchart')  updateStockChart();
  if (activeTab==='watchlist')   updateWatchlistPrices();
}
...
function tickerHTML() {
  const items = mode==='nse'
    ? [{name:'NIFTY 50',key:'NIFTY 50'},{name:'BANKNIFTY',key:'BANKNIFTY'},{name:'FINNIFTY',key:'FINNIFTY'}]
    : [{name:'BTC',key:'BTC'},{name:'ETH',key:'ETH'},{name:'SOL',key:'SOL'},{name:'BNB',key:'BNB'}];

  return items.map(it => {
    const p    = getLTP(it.key);
    const open = sessionOpen[it.key] || p;
    const chg  = open ? ((p-open)/open)*100 : 0;
    const dp   = mode==='nse' ? 2 : (p<10?4:2);
    return `<div class="ticker-item" onclick="selectStock('${it.key}')" style="cursor:pointer" title="View chart">
  ...
  return `<table class="tbl">
    ...
    <tbody>
      ${trades.map((t,i)=>{
        ...
        return `<tr>
          <td>
            <span class="sym-cell">${t.sym}</span>
            ${t.optType?`<span class="badge badge-${t.optType.toLowerCase()}">${t.optType}</span>`:''}
          </td>
          <td><span class="badge badge-${t.type==='options'?'opt':'eq'}">${t.type}</span></td>
          <td><span class="badge badge-${t.dir}">${t.dir}</span></td>
          <td class="num-cell">${mode==='crypto'&&t.type==='equity'?t.qty.toFixed(4):t.qty}</td>
...
function pnlChartTabHTML() {
  return `
    <div class="sec-title">Portfolio P&L — session</div>
    <div class="chart-wrap"><canvas id="pnlCanvas" role="img" aria-label="P&L over time">P&L chart</canvas></div>
  `;
}

function stockChartTabHTML() {
  const syms = mode==='nse'
    ? ['NIFTY 50','BANKNIFTY','FINNIFTY',...NSE_SYMS]
    : CRYPTO_SYMS;
  const p    = getLTP(selectedStock);
  const open = sessionOpen[selectedStock] || p;
  const chg  = open ? ((p-open)/open)*100 : 0;
  return `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px">
      <div style="display:flex;align-items:center;gap:10px">
        <select id="stock-select" onchange="selectStock(this.value)" style="font-size:13px;padding:5px 10px;background:var(--bg3);color:var(--text);border:1px solid var(--border);border-radius:6px">
          ${syms.map(s=>`<option value="${s}" ${s===selectedStock?'selected':''}>${s}</option>`).join('')}
        </select>
        <span style="font-size:22px;font-weight:600;font-family:var(--font-mono);color:var(--text)">${fmt(p,mode)}</span>
        <span style="font-size:13px;font-family:var(--font-mono);color:${chg>=0?'var(--green)':'var(--red)'}">${fmtPct(chg)} today</span>
      </div>
      <div style="display:flex;gap:6px" id="chart-range-btns">
        <button class="seg-btn active" onclick="setChartRange(this,'15m')">15m</button>
        <button class="seg-btn" onclick="setChartRange(this,'30m')">30m</button>
        <button class="seg-btn" onclick="setChartRange(this,'all')">All</button>
      </div>
    </div>
    <div class="chart-wrap" style="height:260px"><canvas id="stockCanvas" role="img" aria-label="Stock price chart for ${selectedStock}">Stock chart</canvas></div>
    <div style="margin-top:16px">
      <div class="sec-title">All prices</div>
      <div id="watchlist-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:6px">
        ${watchlistGridHTML()}
      </div>
    </div>
  `;
...
function watchlistGridHTML() {
  const syms = mode==='nse'
    ? ['NIFTY 50','BANKNIFTY','FINNIFTY',...NSE_SYMS]
    : CRYPTO_SYMS;
  return syms.map(sym => {
    const p    = getLTP(sym);
    const open = sessionOpen[sym] || p;
    const chg  = open ? ((p-open)/open)*100 : 0;
    const dp   = (mode==='crypto'&&p<10) ? 4 : 2;
    return `<div class="wl-card ${sym===selectedStock?'wl-active':''}" onclick="selectStock('${sym}')">
...
function updateWatchlistPrices() {
  const wg = document.getElementById('watchlist-grid');
  if (wg) wg.innerHTML = watchlistGridHTML();
  /* update price header */
  const p    = getLTP(selectedStock);
  const open = sessionOpen[selectedStock] || p;
  const chg  = open ? ((p-open)/open)*100 : 0;
  const priceEl = document.querySelector('#stock-price-header');
  if (priceEl) {
    priceEl.textContent = fmt(p, mode);
  }
}
  ...
  return `
    ...
    ${isOpt?`
      ...
      <div class="form-group">
        ...
        <input id="o-strike" type="number" step="50" placeholder="e.g. 23200" />
      </div>
      <div class="form-group">
    ...
    `:`
      ...
      <div class="form-group">
        ...
        <div id="o-ltp" style="font-size:13px;font-family:var(--font-mono);color:var(--green);padding:4px 0">—</div>
      </div>
      <div class="form-group">
...
function render() {
      ...
      <div class="main">
        <div class="content-panel">
          <div class="content-tabs">
            ...
            <button class="ctab ${activeTab==='stockchart'?'active':''}" onclick="setTab('stockchart')">📈 Live prices</button>
          ...
          ${activeTab==='stockchart' ? stockChartTabHTML()  : ''}
          ${activeTab==='chart'      ? pnlChartTabHTML()    : ''}
        </div>
  ...
  if (activeTab==='chart')      setTimeout(initPnlChart,  60);
  if (activeTab==='stockchart') setTimeout(initStockChart, 60);
...
/* ══════════════════════════════════════════════
   CHARTS
══════════════════════════════════════════════ */
let chartRange = '15m'; /* '15m' | '30m' | 'all' */
...
function getStockChartData() {
  const hist = (stockHistory[selectedStock] || []);
  const now  = Date.now();
  const cutoff = chartRange==='15m' ? 15*60*1000
               : chartRange==='30m' ? 30*60*1000
               : Infinity;
  /* Filter by approximate time window using index (2s per tick) */
  const maxPoints = chartRange==='15m' ? 450
                  : chartRange==='30m' ? 900
                  : hist.length;
  const sliced = hist.slice(-maxPoints);
  return sliced;
}
...
function initStockChart() {
  ...
  const color = last >= first ? '#00c896' : '#ff4f5e';
  stockChart = new Chart(canvas, {
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{
          callbacks:{ label: ctx => fmt(ctx.parsed.y, mode) }
        }
      },
      scales:{
        x:{ ticks:{ font:{size:10,family:'JetBrains Mono'}, maxTicksLimit:6, color:'#555d66' }, grid:{ color:'rgba(255,255,255,0.05)' } },
        y:{
          ticks:{ font:{size:10,family:'JetBrains Mono'}, color:'#555d66', callback:v=>fmt(v,mode,0) },
...
function updateStockChart() {
  ...
  stockChart.data.datasets[0].label  = selectedStock;
  stockChart.data.datasets[0].borderColor = color;
  stockChart.update('none');

  /* Update price header inline */
  const p    = getLTP(selectedStock);
  const open = sessionOpen[selectedStock] || p;
  const chg  = open ? ((p-open)/open)*100 : 0;
  const ph   = document.getElementById('stock-price-display');
  if (ph) {
    ph.innerHTML = `<span style="font-size:22px;font-weight:600;font-family:var(--font-mono);color:var(--text)">${fmt(p,mode)}</span>
      <span style="font-size:13px;font-family:var(--font-mono);color:${chg>=0?'var(--green)':'var(--red)'}">${fmtPct(chg)} today</span>`;
  }
}

window.setChartRange = function(btn, range) {
  chartRange = range;
  document.querySelectorAll('#chart-range-btns .seg-btn').forEach(b=>b.classList.remove('active'));
...
window.selectStock = function(sym) {
  selectedStock = sym;
  if (activeTab !== 'stockchart') { activeTab = 'stockchart'; render(); return; }
  /* Already on stockchart tab — just update the select and reinit chart */
...
function updateOrderLTP() {
  const symEl = document.getElementById('o-sym');
  const ltpEl = document.getElementById('o-ltp');
  if (!symEl || !ltpEl) return;
  const sym = symEl.value === '__custom__'
    ? document.getElementById('o-custom')?.value?.trim().toUpperCase()
    : symEl.value;
  if (!sym || sym === '__custom__') return;
  const p  = getLTP(sym);
  if (p) {
    ltpEl.textContent = fmt(p, mode);
    /* Auto-fill price field if empty */
    const priceEl = document.getElementById('o-price');
    if (priceEl && !priceEl.value) priceEl.value = p.toFixed(mode==='nse'?2:4);
    calcTotal();
  }
}

document.addEventListener('change', e => {
  if (e.target.id === 'o-sym') {
    const cw = document.getElementById('custom-wrap');
    if (cw) cw.style.display = e.target.value==='__custom__' ? 'flex' : 'none';
    updateOrderLTP();
  }
});
  ...
  ov.innerHTML = `
    <div class="modal">
      ...
      <div class="modal-hint">
        Get API key at <a href="https://smartapi.angelbroking.com" target="_blank">smartapi.angelbroking.com</a><br>
        TOTP: My Profile → Security → Authenticator → copy secret key
      </div>
...
/* ══════════════════════════════════════════════
   GLOBAL ACTIONS
══════════════════════════════════════════════ */
window.setMode = function(m) { mode=m; tradeMode='equity'; side='buy'; activeTab='positions'; if(stockChart){stockChart.destroy();stockChart=null;} render(); };
window.setTab  = function(t) { activeTab=t; render(); };
window.setTradeMode = function(m) { tradeMode=m; render(); };
window.setSide = function(s) { side=s; document.getElementById('order-form').innerHTML=orderFormHTML(); updateOrderLTP(); };
window.setOptType = function(t) { optType=t; document.getElementById('order-form').innerHTML=orderFormHTML(); };
...
══════════════════════════════════════════════ */
initPrices();
recordPriceHistory(); /* seed history with initial prices */

if (tickInterval) clearInterval(tickInterval);
tickInterval = setInterval(onTick, 3000);
